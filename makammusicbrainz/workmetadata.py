...
def get_work_metadata(mbid, print_warnings=None):
    work = mb.get_work_by_id(mbid, includes=['artist-rels',
                                             'recording-rels'])['work']

    data = ({'makam': [], 'form': [], 'usul': [], 'mbid': mbid,
             'title': work['title'], 'composer': dict(), 'lyricist': dict()})
